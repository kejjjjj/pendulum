...
#endif
	operator vi2d() { return { (int)x, (int)y }; }
	operator vf2d() { return { (float)x, (float)y }; }
...
struct Pixel
{
	...
	constexpr Pixel(const olc::Pixel& p) { r = p.r, g = p.g, b = p.b, a = p.a; }
...
#endif
	operator olc::Pixel() { return olc::Pixel(r, g, b, a); };
	operator olc::Pixel() const { return olc::Pixel(r, g, b, a); };
